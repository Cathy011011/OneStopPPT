...
function Send-DebugEvent {
    ...
    $envPath = "F:\Codex\PPT自动化\.dbg\powerpoint-com-rejected.env"
    $url = "http://127.0.0.1:7777/event"
    $sessionId = "powerpoint-com-rejected"
    try {
        if (Test-Path -LiteralPath $envPath) {
            foreach ($line in Get-Content -LiteralPath $envPath -ErrorAction Stop) {
                if ($line -like "DEBUG_SERVER_URL=*") { $url = $line.Substring("DEBUG_SERVER_URL=".Length) }
                elseif ($line -like "DEBUG_SESSION_ID=*") { $sessionId = $line.Substring("DEBUG_SESSION_ID=".Length) }
            }
        }
    }
    catch {}
...
function Test-RpcRejected {
    ...
    return ($hresult -eq -2147418111) -or ($message -match "RPC_E_CALL_REJECTED") -or ($message -match "Call was rejected by callee")
}
...
function Invoke-ComWithRetry {
    ...
    while ($true) {
        ...
        try {
            $result = & $Action
            if ($attempt -gt 1) {
                Send-DebugEvent $HypothesisId $Location "$OperationName succeeded after retry." @{ attempt = $attempt; maxAttempts = $MaxAttempts }
            }
            return $result
        }
        catch {
            $isRpcRejected = Test-RpcRejected $_.Exception
            Send-DebugEvent $HypothesisId $Location "$OperationName failed." @{ attempt = $attempt; maxAttempts = $MaxAttempts; retryable = $isRpcRejected; error = $_.Exception.Message }
            if (-not $isRpcRejected -or $attempt -ge $MaxAttempts) {
                throw
            }
...
function Get-PowerPointApplication {
    ...
    try {
        ...
        Send-DebugEvent "A" "powerpoint-bridge.ps1:Get-PowerPointApplication" "Attached to active PowerPoint application." @{ create = $Create; source = "GetActiveObject" }
        return $active
    }
    catch {
        Send-DebugEvent "A" "powerpoint-bridge.ps1:Get-PowerPointApplication" "GetActiveObject failed." @{ create = $Create; error = $_.Exception.Message }
        if (-not $Create) {
            return $null
        }
        try {
            $created = New-Object -ComObject PowerPoint.Application
            Send-DebugEvent "A" "powerpoint-bridge.ps1:Get-PowerPointApplication" "Created new PowerPoint application." @{ create = $Create; source = "New-Object -ComObject" }
            return $created
        }
        catch {
            Send-DebugEvent "A" "powerpoint-bridge.ps1:Get-PowerPointApplication" "Failed to create PowerPoint application." @{ create = $Create; error = $_.Exception.Message }
...
function Invoke-NewPresentation {
    ...
    try {
        Invoke-ComWithRetry "Set application.Visible for new presentation" "powerpoint-bridge.ps1:Invoke-NewPresentation" "B" { $application.Visible = -1 } | Out-Null
        Send-DebugEvent "B" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Set application.Visible for new presentation succeeded." @{ visible = -1 }
    }
    catch {
        Send-DebugEvent "B" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Set application.Visible for new presentation failed." @{ visible = -1; error = $_.Exception.Message }
        throw
    }
    try {
        $presentation = Invoke-ComWithRetry "Presentations.Add" "powerpoint-bridge.ps1:Invoke-NewPresentation" "C" { $application.Presentations.Add($true) }
        Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Presentations.Add succeeded." @{ withWindow = $true; presentations = [int]$application.Presentations.Count }
    }
    catch {
        Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Presentations.Add failed." @{ withWindow = $true; error = $_.Exception.Message }
        throw
    }
    try { $presentation.Windows.Item(1).Activate() } catch {}
    if ([bool](Get-Argument $Arguments "maximize" $true)) {
        try {
            Invoke-ComWithRetry "Set ActiveWindow.WindowState" "powerpoint-bridge.ps1:Invoke-NewPresentation" "D" { $application.ActiveWindow.WindowState = 3 } | Out-Null
            Send-DebugEvent "D" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Set ActiveWindow.WindowState succeeded." @{ windowState = 3 }
        }
        catch {
            Send-DebugEvent "D" "powerpoint-bridge.ps1:Invoke-NewPresentation" "Set ActiveWindow.WindowState failed." @{ windowState = 3; error = $_.Exception.Message }
        }
    }
...
function Invoke-Launch {
    ...
    try {
        Invoke-ComWithRetry "Set application.Visible during launch" "powerpoint-bridge.ps1:Invoke-Launch" "B" { $application.Visible = $visibleValue } | Out-Null
        Send-DebugEvent "B" "powerpoint-bridge.ps1:Invoke-Launch" "Set application.Visible during launch succeeded." @{ visible = $visible; visibleValue = $visibleValue }
    }
    catch {
        Send-DebugEvent "B" "powerpoint-bridge.ps1:Invoke-Launch" "Set application.Visible during launch failed." @{ visible = $visible; visibleValue = $visibleValue; error = $_.Exception.Message }
        throw
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$filePath)) {
        ...
        if ($null -eq $presentation) {
            ...
            try {
                $presentation = Invoke-ComWithRetry "Presentations.Open" "powerpoint-bridge.ps1:Invoke-Launch" "C" { $application.Presentations.Open($resolved, $readOnly, $false, $visible) }
                Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-Launch" "Presentations.Open succeeded." @{ file = $resolved; readOnly = $readOnly; visible = $visible }
            }
            catch {
                Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-Launch" "Presentations.Open failed." @{ file = $resolved; readOnly = $readOnly; visible = $visible; error = $_.Exception.Message }
                throw
            }
        }
    }
        ...
        if ($null -eq $presentation -and [bool](Get-Argument $Arguments "create_if_missing" $true)) {
            try {
                $presentation = Invoke-ComWithRetry "Presentations.Add during launch" "powerpoint-bridge.ps1:Invoke-Launch" "C" { $application.Presentations.Add($visible) }
                Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-Launch" "Presentations.Add during launch succeeded." @{ visible = $visible; presentations = [int]$application.Presentations.Count }
            }
            catch {
                Send-DebugEvent "C" "powerpoint-bridge.ps1:Invoke-Launch" "Presentations.Add during launch failed." @{ visible = $visible; error = $_.Exception.Message }
                throw
            }
        }
    }
    ...
    if ([bool](Get-Argument $Arguments "maximize" $true)) {
        try {
            Invoke-ComWithRetry "Set ActiveWindow.WindowState during launch" "powerpoint-bridge.ps1:Invoke-Launch" "D" { $application.ActiveWindow.WindowState = 3 } | Out-Null
            Send-DebugEvent "D" "powerpoint-bridge.ps1:Invoke-Launch" "Set ActiveWindow.WindowState during launch succeeded." @{ windowState = 3 }
        }
        catch {
            Send-DebugEvent "D" "powerpoint-bridge.ps1:Invoke-Launch" "Set ActiveWindow.WindowState during launch failed." @{ windowState = 3; error = $_.Exception.Message }
        }
    }
    $summary = Get-PresentationSummary $application $presentation
    $summary.connected = $true
